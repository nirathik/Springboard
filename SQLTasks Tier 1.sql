...
SELECT name, 
monthlymaintenance, 
CASE WHEN monthlymaintenance > 100 THEN 'Expensive' WHEN monthlymaintenance <= 100 THEN 'Cheap' END AS category 
FROM Facilities;
...
AND (sub2.slotNumber * f.membercost > 30) THEN sub2.slotNumber * f.membercost WHEN sub2.Type = 'Guest' 
AND (sub2.slotNumber * f.guestcost > 30) THEN sub2.slotNumber * f.guestcost END AS Cost 
FROM (SELECT DISTINCT (CONCAT(m.firstName, ' ', m.surname)) AS membername, 
...
LEFT JOIN Members m ON m.memid = sub1.memid) AS sub2 
LEFT JOIN Facilities f ON f.facid = sub2.facilityId) AS sub3 
WHERE sub3.Cost > 30 
